...
exports.addCommetChat = async (req, res) => {
  ...
  try {
    const aiPrompt = `너는 역사적으로 유명한 인물 ${star}야. 
    너의 시대적 배경과 가치관을 고려하여, "${comment}"라는 댓글이 달렸어.  
    이 댓글에 대해 네 시대의 말투로 자연스럽고 재치 있는 30자 내외의 답글을 작성해줘.  
    
    - 반드시 **한글**로만 답변해야 해.  
    - 답변은 **반말이 아닌**, 당시 시대의 높임말과 말투를 반영해야 해.  
    - 대답은 반드시 **JSON 형식**으로 응답해야 해.  
    - 예제: {"response": "백성들에게 한글을 쉽게 가르치는 것이 중요하오."}`;
